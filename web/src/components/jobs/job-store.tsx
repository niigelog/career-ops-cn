  ...
  useEffect(() => {
    try {
      ...
      if (Array.isArray(arr)) {
        ...
        setJobs(arr.map((j: Job) => (j.status === "running" ? { ...j, status: "error", steps: [...(j.steps || []), { kind: "status", label: t.engineStatus.interrupted, ts: Date.now() }] } : j)));
      }
    } catch {
      /* ignore */
    }
  ...
  const startJob = useCallback(
    (opts: StartOpts): string | null => {
      ...
      const job: Job = {
        ...
        status: "running",
        steps: [{ kind: "status", label: t.engineStatus.starting, ts: Date.now() }],
        text: "",
        startedAt: Date.now(),
      };
      setJobs((js) => [job, ...js]);
      ...
      if (!cliId) {
        patch(id, (j) => ({ ...j, status: "error", endedAt: Date.now(), steps: [...j.steps, { kind: "status", label: t.engineStatus.noCli, ts: Date.now() }] }));
        return id;
      }
      ...
      (async () => {
        ...
        try {
          ...
          if (!res.ok || !res.body) {
            ...
            finish("error", e.error || t.engineStatus.failedToStart);
            return;
          }
          ...
          finish("error", t.engineStatus.connectionError);
        }
      })();
