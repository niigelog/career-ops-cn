...
export function ExploreProvider({ children }: { children: React.ReactNode }) {
  ...
  const discover = useCallback(async () => {
    ...
    setPartial(false);
    setError("");
    setStatus(t.engineStatus.castingNet);
    ...
    try {
      ...
      if (r.status === 400) {
        ...
        sawError = d.error || t.engineStatus.scannerUnavailable;
      } else if (!r.body) {
        sawError = t.engineStatus.noStream;
      } else {
  ...
  const loadFresh = useCallback(async () => {
    ...
    runningRef.current = true;
    setPhase("casting");
    setStatus(t.engineStatus.loadingFresh);
    setOffers([]);
    setMatchCount(0);
    setCompaniesScanned(0);
    setCompaniesAvailable(0);
    ...
    try {
      ...
      if (!d || !Array.isArray(d.offers)) {
        setError(t.engineStatus.loadFreshUnexpected);
        setPhase("failed");
        return;
      }
      ...
      setError(e instanceof Error ? e.message : t.engineStatus.loadFreshFailed);
      setPhase("failed");
    } finally {
      runningRef.current = false;
    }
  ...
  const discoverAI = useCallback(async () => {
    ...
    setAiCost({ searches: 0, candidates: 0, fetches: 0 });
    setError("");
    setStatus(t.engineStatus.castingWeb);
