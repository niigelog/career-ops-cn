...
function freshness(postedAt: string, t: Dict): string {
  ...
  return days === 0 ? t.freshness.today : days === 1 ? t.freshness.dayAgo : t.freshness.daysAgo(days);
}
...
// Worker KINDS are protocol values (see the jobs store) — only the label moves.
const WORKER_LABEL_KEYS: Record<string, keyof Dict["worker"]> = {
  evaluate: "evaluating",
  pdf: "preparingCv",
...
export function DiscoveryCard({ offer, inPipeline, evaluatedN }: { offer: DiscoveredOffer; inPipeline: boolean; evaluatedN?: string }) {
  ...
  const labelKey = WORKER_LABEL_KEYS[job?.kind ?? ""];
  const statusLabel = labelKey ? (t.worker[labelKey] as string) : t.worker.working;
  ...
  const fresh = freshness(offer.postedAt, t) || offer.postedHint || "";

  const evaluate = () => {
    addToPipeline([offer]); // evaluating implies it's in the pipeline — record it
    startJob({ title: t.worker.evaluateTitle(offer.company), subtitle: offer.title, kind: "evaluate", input: offer.url, page: "/explore" });
  };
    ...
    <div className="co-rise group flex min-w-0 flex-col gap-2.5 rounded-xl border border-border bg-surface/40 p-3.5 text-left transition-all hover:-translate-y-0.5 hover:border-brand/30 hover:shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <a
          ...
          title={t.discovery.openPosting}
          aria-label={t.discovery.openPosting}
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
        ...
        {unverified && (
          <span
            ...
            title={t.discovery.unverifiedHint}
          >
            <ShieldQuestion className="size-3" /> {t.discovery.unverified}
          </span>
        )}
        {offer.matchedKeyword && (
          <span className="text-faint" title={t.discovery.matchedHint}>
            {t.discovery.matchedPrefix} <span className="text-brand/80">{offer.matchedKeyword}</span>
          </span>
        )}
      </div>
      ...
      <div className="mt-0.5">
        {evaluatedN || doneEval ? (
          ...
          >
            <Check className="size-3.5" /> {t.discovery.evaluatedViewReport}
          </a>
        ) : working ? (
          <div className="inline-flex w-full items-center justify-center gap-1.5 rounded-md border border-brand/30 bg-brand-soft/60 px-2.5 py-2 text-xs font-medium text-brand">
            ...
            {statusLabel}
            <span className="text-brand/60">{t.discovery.inPipelineSuffix}</span>
          </div>
        ) : (
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {isAdded ? t.discovery.inPipeline : t.discovery.addToPipeline}
            </button>
            <button
              type="button"
              onClick={evaluate}
              title={unverified ? t.discovery.evaluateHintUnverified : t.discovery.evaluateHint}
            ...
            >
              {t.discovery.evaluate} <Coins className="size-3.5 opacity-80" />
            </button>
          </div>
        )}
      </div>
