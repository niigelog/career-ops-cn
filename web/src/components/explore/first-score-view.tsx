...
function extractWhy(job: Job): string | null {
  ...
  return para ? para.slice(0, 240) : null; // caller falls back to the localized line
}
...
export function FirstScoreView() {
  ...
  useEffect(() => {
    ...
    const focusTimer = window.setTimeout(() => panelRef.current?.querySelector<HTMLElement>("a, button")?.focus(), 40);
  ...
  const why = extractWhy(firstDone) ?? t.results.strongMatchFallback;
  ...
  return (
    <div className="co-aha" role="dialog" aria-modal="true" aria-label={t.results.firstScoreTitle} onClick={close}>
      <style>{STYLE}</style>
      <div ref={panelRef} className="co-aha__card" onClick={(e) => e.stopPropagation()}>
        <div className="co-aha__glow" />
        <button onClick={close} aria-label={t.common.close} className="absolute right-3 top-3 z-10 rounded-md p-1.5 text-faint transition hover:text-foreground">
          <X className="size-4" />
        </button>
        ...
        <div className="relative px-7 pb-7 pt-8">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            >
              <FileText className="size-4" /> {t.results.seeFullReport}
            </button>
            <button
              onClick={() => {
                close();
            ...
            >
              <Compass className="size-4" /> {t.results.findMoreLikeThis}
            </button>
          </div>
        </div>
      </div>
