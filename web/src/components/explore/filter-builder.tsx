...
// Same windows as the inbox facets — the 24h option reads as hours, the rest as
// days (localized in the render; see t.inbox.windowHours / windowDays).
const RECENCY = [
  { days: 1, unit: "hours" as const },
  { days: 3, unit: "days" as const },
  { days: 7, unit: "days" as const },
  { days: 14, unit: "days" as const },
  { days: 30, unit: "days" as const },
];
...
export function FilterBuilder({
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <Label hint={filters.positive.length === 0 ? t.filters.rolesHintEmpty : undefined}>{t.filters.rolesToFind}</Label>
        <KeywordField values={filters.positive} tone="inc" placeholder={t.filters.rolesPlaceholder} onChange={(v) => set({ positive: v })} />
        {seededFrom.length > 0 && filters.positive.length > 0 && (
          <p className="mt-1 text-[11px] text-faint">{t.filters.seededFrom(seededFrom.join(" + "))}</p>
        )}
      </div>

      <div>
        <Label>{t.filters.exclude}</Label>
        <KeywordField values={filters.negative} tone="exc" placeholder={t.filters.excludePlaceholder} onChange={(v) => set({ negative: v })} />
      </div>

      <div className="flex flex-wrap items-end gap-x-8 gap-y-4">
        <div>
          <Label hint={t.filters.postedWithinHint}>
            <span className="inline-flex items-center gap-1.5">
              <Clock className="size-3.5 text-muted" /> {t.filters.postedWithin}
            </span>
          </Label>
          <div className="inline-flex rounded-lg border border-border bg-surface/40 p-0.5">
            {RECENCY.map((r) => (
              ...
              >
                {r.unit === "hours" ? t.inbox.windowHours(r.days * 24) : t.inbox.windowDays(r.days)}
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          <Label hint={filters.ats.length === 0 ? t.filters.sourcesHintEmpty : undefined}>{t.filters.sources}</Label>
      ...
      {advanced && (
        <div className="space-y-3 rounded-xl border border-border bg-surface/30 p-3">
          <div className="flex items-center gap-1.5 text-[12px] text-muted">
            <MapPin className="size-3.5" /> {t.filters.location}
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
            <div>
              <Label hint={t.filters.alwaysIncludeHint}>{t.filters.alwaysInclude}</Label>
              <KeywordField values={filters.alwaysAllow} tone="inc" placeholder={t.filters.alwaysIncludePlaceholder} onChange={(v) => set({ alwaysAllow: v })} />
            </div>
            <div>
              <Label>{t.filters.onlyIn}</Label>
              <KeywordField values={filters.allow} tone="inc" placeholder={t.filters.onlyInPlaceholder} onChange={(v) => set({ allow: v })} />
            </div>
            <div>
              <Label>{t.filters.neverIn}</Label>
              <KeywordField values={filters.block} tone="exc" placeholder={t.filters.neverInPlaceholder} onChange={(v) => set({ block: v })} />
            </div>
          </div>
          <div>
            <Label hint={t.filters.scanDepthHint(filters.limitPerAts)}>{t.filters.scanDepth}</Label>
            <input
              type="range"
              min={50}
              max={500}
