...
export function ApplyView() {
              ...
              <div className="min-w-0">
                ...
                {a.url && /^https?:\/\//.test(a.url) && (
                  <a href={a.url} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-brand hover:underline">
                    {t.apply.openDirectly} <ExternalLink className="size-3" />
                  </a>
                )}
              </div>
            </div>
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      {opening && (
        <>
          <ProcessingHero title={t.apply.readingForm} subtitle={t.apply.readingFormSub} />
          <FieldSkeleton />
        </>
      )}
      ...
      {!busy && (
        <div className="co-rise">
          ...
          <div className="mb-4 flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs text-muted">{t.apply.orAskAssistant}</span>
          </div>
          ...
          {(prefilling || a.prefillLog.length > 0) && (
            <details className="mb-4 rounded-lg border border-border bg-surface/60 backdrop-blur-sm" open={false}>
              <summary className="flex cursor-pointer select-none items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-muted">
                ...
                <span className="ml-auto text-faint">{t.apply.steps(a.prefillLog.length)}</span>
              </summary>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            ...
            >
              ...
              {filling ? t.apply.fillingRealForm : t.apply.fillAndReview}
            </button>
            <button
              onClick={a.agentFill}
              disabled={filling || prefilling}
              title={t.apply.letAiFillTooltip}
            ...
            >
              <MousePointerClick className="size-4" /> {t.apply.letAiFill}
            </button>
          ...
          {done && (
            <div className="co-rise mt-4 flex items-start gap-2.5 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm backdrop-blur-sm">
              ...
              <div>
                <span className="font-medium text-emerald-700 dark:text-emerald-400">{t.apply.formInFront}</span>{" "}
                <span className="text-muted">{t.apply.reviewYourself}</span>
              </div>
            </div>
          )}
        </div>
  ...
  async function markApplied() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(d.error || t.apply.markFailed);
        setMarking(false);
        return;
      }
      leave();
      ...
      setError(t.apply.markUnconfirmed);
      setMarking(false);
    }
  }
  ...
  if (confirming) {
    return (
      <div className="co-rise mt-8 rounded-xl border border-amber-500/40 bg-amber-500/10 p-4 backdrop-blur-sm">
        <p className="text-sm font-medium text-foreground">{t.apply.leaveTitle}</p>
        <p className="mt-1 text-xs text-muted">{t.apply.leaveBody}</p>
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          >
            <ArrowLeft className="size-3.5" /> {t.apply.leaveDiscard}
          </button>
          <button
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {a.n && (
          <button
            onClick={markApplied}
            disabled={marking}
            title={t.apply.markAppliedTooltip(String(a.n))}
        ...
        {a.n && <span className="text-xs text-muted">{t.apply.markAppliedHint}</span>}
      </div>
...
// Action KINDS come from the drive protocol; only the verb is localized.
const DRIVE_VERB_KEYS: Record<string, keyof Dict["apply"]> = {
  click: "verbClick",
  type: "verbType",
  select: "verbSelect",
  scroll: "verbScroll",
  "parse-error": "verbThinking",
  stuck: "verbStuck",
  reached_form: "verbReachedForm",
};
  ...
  return (
    <div className="co-rise">
      <div className="flex flex-col items-center gap-3 py-7 text-center">
        ...
        <div className="font-display text-2xl text-landing">{filling ? t.apply.aiFilling : t.apply.reachingForm}</div>
        <p className="max-w-sm text-sm text-muted">{filling ? t.apply.aiFillingBody : t.apply.reachingFormBody}</p>
      </div>
      {last?.thumb ? (
        // eslint-disable-next-line @next/next/no-img-element
        ...
        <ol className="mt-3 space-y-1.5 rounded-xl border border-border/70 bg-surface/70 p-3 backdrop-blur-sm">
          {steps.map((s, i) => (
            <li key={i} className={cn("flex items-center gap-2 text-xs", i === steps.length - 1 ? "text-foreground" : "text-muted")}>
              ...
              <span className="shrink-0 font-medium">{DRIVE_VERB_KEYS[s.action] ? (t.apply[DRIVE_VERB_KEYS[s.action]] as string) : s.action}</span>
...
function PhaseRail({ phase }: { phase: number }) {
  const t = useT();
  const steps = [
    { label: t.apply.railRead, icon: ScanLine },
    { label: t.apply.railDraft, icon: PenLine },
    { label: t.apply.railReview, icon: CheckCircle2 },
  ];
  return (
...
function RotatingStatus() {
  ...
  const msgs = [
    t.apply.rotatingCv,
    t.apply.rotatingRole,
    t.apply.rotatingMatch,
    t.apply.rotatingWrite,
    t.apply.rotatingFlag,
  ];
  const [i, setI] = useState(0);
  useEffect(() => {
    const timer = setInterval(() => setI((n) => (n + 1) % msgs.length), 2800);
  ...
  return (
    <div className={flash ? "co-flash" : ""} style={flash ? { animationDelay: `${Math.min(index * 70, 900)}ms` } : undefined}>
      ...
      {writing ? (
        ...
        <textarea rows={3} maxLength={f.maxLength} value={value} onChange={(e) => onChange(e.target.value)} placeholder={needs ? t.apply.youFillThis : "…"} className={cn(base, "resize-none")} />
        ...
        /resume|résumé|\bcv\b|curriculum|currículum|lebenslauf/i.test(f.label || "") ? (
          <div className="flex items-center gap-2 rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-700 dark:text-emerald-400">
            <FileCheck2 className="size-4 shrink-0" /> {t.apply.cvAutoAttached}
          </div>
        ) : (
          <div className="flex items-center gap-2 rounded-lg border border-dashed border-border px-3 py-2 text-sm text-muted">
            <Paperclip className="size-4 shrink-0" /> {t.apply.attachYourself}
          </div>
        )
      ) : (
        <input type={["email", "tel", "url", "number", "date"].includes(f.type) ? f.type : "text"} maxLength={f.maxLength} value={value} onChange={(e) => onChange(e.target.value)} placeholder={needs ? t.apply.youFillThis : "…"} className={base} />
      )}
    </div>
  );
}
