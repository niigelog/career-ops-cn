...
// Engine values (followup-cadence.mjs); the label comes from the catalog.
...
const COLUMNS = [
  ...
  { key: "score", labelKey: "colScore" },
  { key: "status", labelKey: "colStatus" },
  { key: "urgency", labelKey: "colUrgency" },
  { key: "days", labelKey: "colDays" },
  { key: "next", labelKey: "colNext" },
  { key: "count", labelKey: "colCount" },
  { key: "since", labelKey: "colSince" },
] as const;
...
export function FollowupsView() {
  ...
  const removeLogged = async (num: number) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setActionError(typeof j.error === "string" ? t.followups.removeFailedWith(j.error) : t.followups.removeFailed);
      }
    } catch {
      setActionError(t.followups.removeFailed);
    }
    refetch();
  };
  ...
  const subtitle = !data ? (
    ...
    <>
      <span className="tabular-nums">{meta.actionable}</span> {t.followups.active} ·{" "}
      <span className="tabular-nums">{meta.urgent}</span> {t.followups.urgent} ·{" "}
      <span className="tabular-nums">{meta.overdue}</span> {t.followups.overdue}
    </>
  );
  ...
  return (
    <div className="mx-auto max-w-none px-6 py-8">
      ...
      {meta && !filtering && <NarrativeCard t={t} meta={meta} entries={entries} />}

      {/* urgency filter */}
      <div className="mt-6 flex flex-wrap gap-1 border-b border-border">
        {URGENCY_TABS.map((u) => {
          const count = u === "ALL" ? entries.length : entries.filter((e) => e.urgency.toUpperCase() === u).length;
          return (
            <button
              key={u}
              onClick={() => setParams({ urgency: u === "ALL" ? null : u })}
              className={cn(
                "-mb-px border-b-2 px-3 py-2 text-xs font-medium transition-colors",
                tab === u ? "border-brand text-foreground" : "border-transparent text-muted hover:text-foreground",
              )}
            >
              {t.followups[URGENCY_LABEL_KEYS[u]]} <span className="text-faint tabular-nums">{count}</span>
            </button>
          );
        })}
      </div>
      ...
      {!data ? null : !data.available ? (
        <EmptyPanel title={t.followups.unavailable} body={t.followups.unavailableBody} />
      ) : filtered.length === 0 ? (
        filtering ? (
          <EmptyPanel title={t.followups.noMatches} body={t.followups.noMatchesBody} />
        ) : (
          <EmptyPanel title={t.followups.nothingToChase} body={t.followups.nothingToChaseBody} />
        )
      ) : (
        <div className="mt-4 overflow-x-auto rounded-2xl border border-border">
          <table className="w-full min-w-[880px] text-sm">
            <thead className="bg-surface/60 text-left text-xs uppercase tracking-wide text-faint">
              <tr>
                <th className="w-8 px-2 py-2.5" aria-label={t.followups.expand} />
                {COLUMNS.map((c) => {
                ...
                <th className="px-2.5 py-2.5 font-medium">{t.followups.colAction}</th>
              </tr>
            </thead>
...
function NarrativeCard({ t, meta, entries }: { t: Dict; meta: CadenceMetadata; entries: CadenceEntry[] }) {
  ...
  const parts: string[] = [];
  if (meta.overdue > 0) parts.push(t.followups.narrativeOverdue(meta.overdue));
  if (meta.urgent > 0) parts.push(t.followups.narrativeUrgent(meta.urgent));
  if (pressing.length > 0) {
    parts.push(t.followups.narrativePressing(oxfordJoin(pressing.map((e) => `${e.company} (#${e.num})`))));
    const days = pressing.map((e) => e.daysSinceApplication);
    const max = Math.max(...days);
    parts.push(days.every((d) => d === max) ? t.followups.narrativeAllDays(max) : t.followups.narrativeUpToDays(max));
  }
...
function FollowupRow({
  ...
  const statusText = statusLabel(t, e.status);
      ...
      <tr className="group transition-colors hover:bg-surface/40">
        <td className="px-2 py-3">
          <button
            ...
            aria-label={t.followups.historyToggle(!expanded, e.company)}
        ...
        <td className="px-2.5 py-3">
          <Badge tone={followupStatusTone(e.status)}>{statusText}</Badge>
        </td>
        <td className="px-2.5 py-3">
          <Badge tone={urgencyTone(e.urgency)}>{t.followups[URGENCY_LABEL_KEYS[e.urgency.toUpperCase() as keyof typeof URGENCY_LABEL_KEYS] ?? "urgencyAll"]}</Badge>
        </td>
        ...
        <td className="whitespace-nowrap px-2.5 py-3">
          {e.daysUntilNext == null ? (
            ...
            <span className={cn(e.daysUntilNext < 0 && "font-medium text-red-600 dark:text-red-400")} title={e.nextFollowupDate ?? undefined}>
              {relativeDays(e.daysUntilNext, t)}
            </span>
          )}
          {e.nextOverride && (
            <span
              className="ml-1.5 inline-flex align-[-1px]"
              title={t.followups.pinnedTo(e.nextOverride)}
              aria-label={t.followups.pinnedManually}
            >
        ...
        <td className="whitespace-nowrap px-2.5 py-3">
          <span className="inline-flex items-center gap-0.5">
            ...
            <button
              type="button"
              onClick={onPin}
              title={e.nextOverride ? t.followups.pinnedTooltip(e.nextOverride) : t.followups.pinTooltip}
              className={cn(
      ...
      {expanded && (
        <tr className="bg-surface/30">
          <td colSpan={COLUMNS.length + 2} className="px-4 py-3">
            <HistoryPanel t={t} entry={e} onRemove={onRemove} />
          </td>
        </tr>
      )}
    </>
...
function HistoryPanel({ t, entry: e, onRemove }: { t: Dict; entry: CadenceEntry; onRemove: (num: number) => void }) {
  ...
  return (
    <div className="space-y-2 pl-7 text-sm">
      {history.length === 0 ? (
        <p className="text-faint">{t.followups.noneLogged}</p>
      ) : (
              ...
              <span className="inline-flex w-5 shrink-0 justify-center self-center">
                {f.num != null && (
                  <button
                    ...
                    title={t.followups.removeLogged}
                    aria-label={t.followups.removeLoggedOn(f.date)}
