...
export function NextDateDialog({
  ...
  const call = async (method: "POST" | "DELETE", body: Record<string, unknown>, kind: "set" | "clear") => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(typeof j.error === "string" ? j.error : t.dialogs.pinFailed);
        setBusy(null);
        return;
      }
      onChanged();
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-label={t.dialogs.pinAria(entry.company)}
        className="w-full max-w-sm rounded-2xl border border-border bg-surface p-5 shadow-xl"
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="font-display text-lg">{t.dialogs.pinTitle}</h2>
        ...
        >
          ...
          <div className="flex gap-2">
            {[3, 7, 14].map((n) => (
              ...
              >
                {t.dialogs.plusDays(n)}
              </button>
            ))}
          </div>
          <p className="text-xs leading-relaxed text-faint">
            {t.dialogs.pinExplainer}
          </p>
          ...
          <div className="flex items-center justify-end gap-2 pt-1">
            {entry.nextOverride && (
              <button
                ...
                title={t.dialogs.currentlyPinned(entry.nextOverride)}
              >
                {busy === "clear" ? <Loader2 className="size-3.5 animate-spin" /> : <PinOff className="size-3.5" />} {t.dialogs.clearPin}
              </button>
            )}
            ...
            >
              {busy === "set" ? <Loader2 className="size-3.5 animate-spin" /> : <Pin className="size-3.5" />} {t.dialogs.pinDate}
            </button>
          </div>
        </form>
      </div>
