  ...
  return (
    <div className="mx-auto max-w-none px-6 py-8">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl tracking-tight text-landing">{t.cv.editorTitle}</h1>
          <p className="mt-1 text-sm text-muted">
            {t.cv.editLead} <code className="text-foreground">cv.md</code> {t.cv.editTail}
            {!exists && loaded && <span className="ml-1 text-faint">{t.cv.noCvYet}</span>}
          </p>
        </div>
        <button
          type="button"
        ...
        >
          ...
          {saved ? t.common.saved : t.cv.save}
        </button>
      </div>

      {!loaded ? (
        <div className="mt-6 text-sm text-muted">{t.common.loading}</div>
      ) : (
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          <textarea
            ...
            spellCheck={false}
            placeholder={t.engineStatus.cvPlaceholder}
          ...
          <article className="report-prose min-h-[60vh] overflow-auto rounded-2xl border border-border bg-surface/30 p-5">
            {content.trim() ? (
              ...
              <p className="text-muted">{t.cv.previewHere}</p>
            )}
          </article>
        </div>
      )}
