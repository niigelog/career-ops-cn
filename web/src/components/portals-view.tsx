...
const TONE: Record<string, { dot: string; labelKey: "toneLive" | "toneEmpty" | "toneBroken" | "toneSkipped"; chip: string }> = {
  live: { dot: "bg-emerald-500", labelKey: "toneLive", chip: "bg-emerald-500/15 text-emerald-700 dark:text-emerald-400" },
  empty: { dot: "bg-amber-500", labelKey: "toneEmpty", chip: "bg-amber-500/15 text-amber-700 dark:text-amber-400" },
  broken: { dot: "bg-red-500", labelKey: "toneBroken", chip: "bg-red-500/15 text-red-700 dark:text-red-400" },
  skipped: { dot: "bg-zinc-400", labelKey: "toneSkipped", chip: "bg-surface-hover text-muted" },
};
    ...
    <div>
      ...
      {res && !res.available && (
        <p className="mt-4 rounded-xl border border-dashed border-border bg-surface/30 p-4 text-sm text-muted">
          <code className="text-foreground">verify-portals.mjs</code> {t.portals.validatorMissingLead}
        </p>
      )}
      {res && res.available && !res.configured && (
        <p className="mt-4 rounded-xl border border-dashed border-border bg-surface/30 p-4 text-sm text-muted">
          {t.portals.noPortalsLead} <code className="text-foreground">portals.yml</code> {t.portals.noPortalsTail}
        </p>
      )}
      ...
      {res && res.configured && (
        <div className="mt-5">
          <p className="text-sm text-muted">
            <span className="tabular-nums text-emerald-600 dark:text-emerald-400">{liveN}</span> {t.portals.live} ·{" "}
            <span className="tabular-nums text-red-600 dark:text-red-400">{broken.length}</span> {t.portals.broken} ·{" "}
            <span className="tabular-nums">{companies.length}</span> {t.portals.tracked}
          </p>
          {broken.length > 0 && (
            <div className="mt-3 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm">
              <span className="font-medium text-red-700 dark:text-red-400">
                {t.portals.silentDrop(broken.length)}
              </span>{" "}
              <span className="text-muted">
                {t.portals.brokenLead} <code>careers_url</code> {t.portals.brokenMid} <code>portals.yml</code>{" "}
                {t.portals.brokenTail}
              </span>
            </div>
          )}
              ...
              return (
                <li key={c.name} className="flex items-center gap-3 px-4 py-2.5">
                  ...
                  <div className="ml-auto flex shrink-0 items-center gap-2">
                    {c.status === "broken" && <FixAffordance company={c.name} job={fixByCompany.get(c.name)} onFix={() => startJob({ title: t.portals.fixTitle(c.name), subtitle: t.portals.fixSubtitle, kind: "fix-portal", input: c.name, page: "/portals" })} />}
                    <span className={cn("rounded px-1.5 py-0.5 text-[10px] font-semibold", tone.chip)}>{t.portals[tone.labelKey]}</span>
                  </div>
                </li>
              );
            })}
...
function FixAffordance({ company, job, onFix }: { company: string; job?: Job; onFix: () => void }) {
  ...
  if (job?.status === "running")
    return (
      <Link href={`/jobs/${job.id}`} className="inline-flex items-center gap-1 text-xs font-medium text-brand">
        <Loader2 className="size-3 animate-spin" /> {t.portals.fixing}
      </Link>
    );
  if (job?.status === "done")
    return (
  ...
  return (
    <button
      onClick={onFix}
      title={t.portals.repairTooltip(company)}
    ...
    >
      <Wrench className="size-3" /> {t.portals.fix}
    </button>
  );
}
