...
export function GeneratePdfButton({ n, company, pdfReady }: { n: string; company: string; pdfReady: boolean }) {
  ...
  const generate = () =>
    startJob({ title: t.actions.cvPdfTitle(company), subtitle: t.actions.cvPdfSubtitle, kind: "pdf", input: n, page: `/pipeline/${n}` });
  ...
  if (ready)
    return (
      <span className="inline-flex items-center gap-1">
        ...
        >
          <FileText className="size-3.5" /> {t.actions.viewTailoredCv}
        </a>
        <button
          onClick={generate}
          title={t.actions.regenerateCv}
  ...
  return (
    <span className="inline-flex items-center gap-1.5">
      <button
        ...
        title={t.actions.generateCvTooltip}
      >
        <FileDown className="size-3.5" /> {t.actions.generateCv}
      </button>
