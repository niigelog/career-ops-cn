    ...
    <div className="mx-auto max-w-none px-6 py-8">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            <span className="tabular-nums">{pendingInbox.length}</span> {t.pipeline.inInbox} ·{" "}
            <span className="tabular-nums">{applications.length}</span> {t.pipeline.tracked}
          </p>
        </div>
      ...
      <div className="mt-6 flex flex-wrap gap-1 border-b border-border">
        {TABS.map((tabKey) => {
          ...
          return (
            ...
            >
              {statusLabel(t, tabKey)} <span className="text-faint tabular-nums">{count}</span>
            </button>
          );
        })}
      </div>
      ...
      {tab !== "INBOX" && minFilter != null && (
        <div className="mt-3 flex items-center gap-2">
          ...
          >
            {t.pipeline.scoreAtLeast(minFilter.toFixed(1))}
            <X className="size-3" />
          </button>
        </div>
      )}
      ...
      {tab === "INBOX" ? (
        ...
        pendingInbox.length > 0 ? (
          ...
          <InboxEmpty count={0} filtered={false} t={t} />
        )
      ) : filtered.length > 0 ? (
        /* ── Tracker table ──
                ...
                <tr key={`${r.n}-${i}`} className="group transition-colors hover:bg-surface/40">
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-muted">
                    <span className="inline-flex items-center gap-1.5">
                      ...
                      {statusLabel(t, canonStatus(r.status))}
                    </span>
                  </td>
...
function InboxEmpty({ count, filtered, t }: { count: number; filtered: boolean; t: Dict }) {
  if (filtered) {
    return (
      <div className="mt-4 rounded-2xl border border-dashed border-border bg-surface/30 px-6 py-12 text-center">
        <p className="font-display text-lg">{t.pipeline.noMatches}</p>
        <p className="mx-auto mt-1 max-w-sm text-sm text-muted">{t.pipeline.noMatchesClearSearch}</p>
      </div>
    );
  }
  return (
    <div className="dot-bg mt-4 overflow-hidden rounded-2xl border border-border bg-surface/50 bg-origin-border bg-gradient-to-tr from-brand/10 via-transparent to-transparent shadow-lg">
      <div className="flex items-center gap-2 border-b border-foreground/10 px-5 py-3">
        ...
        <span className="size-2.5 rounded-full bg-foreground/15" aria-hidden="true" />
        <span className="size-2.5 rounded-full bg-foreground/15" aria-hidden="true" />
        <span className="ml-3 font-mono text-xs tracking-wide text-muted">{t.pipeline.inboxChrome}</span>
      </div>
      <div className="px-6 py-10 text-center">
        <p className="font-display text-lg">
          {t.pipeline.inboxEmptyLead} <span className="text-brand">{t.pipeline.inboxEmptyWord}</span> {t.pipeline.inboxEmptyTail}
        </p>
        {count > 0 ? (
          ...
          <>
            ...
            <p className="mx-auto mt-4 max-w-sm text-xs text-muted">
              {t.pipeline.preferTerminalLead} <code className="rounded bg-surface-hover px-1 py-0.5 font-mono">career-ops scan</code>{t.pipeline.preferTerminalMid}{" "}
              <code className="rounded bg-surface-hover px-1 py-0.5 font-mono">data/pipeline.md</code>{t.pipeline.preferTerminalTail}
            </p>
          </>
        )}
      </div>
