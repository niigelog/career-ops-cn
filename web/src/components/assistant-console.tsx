...
// The opening line is UI copy (t.assistant.greeting). It is also the marker that
// tells the send path "this is the canned opener, don't replay it as history",
// so the comparison below must use the SAME localized string, not a constant.
...
// the user is looking at. This text goes into the MODEL's system context, not the
// UI — it stays English on purpose (the assistant answers in the user's language
// because config/profile.yml `language.output` tells it to, per AGENTS.md).
...
export function AssistantConsole() {
  ...
  const suggestions = useMemo(() => {
    ...
    if (rep) {
      chips.push({ label: t.assistant.chipWhyScoreLabel, send: t.assistant.chipWhyScoreSend });
      chips.push({ label: t.assistant.chipShouldApplyLabel, send: t.assistant.chipShouldApplySend });
      chips.push({ label: t.assistant.chipCoverLetterLabel, send: t.assistant.chipCoverLetterSend });
      return chips;
    }
    const pending = pipeline.inbox.filter((j) => !j.done);
    if (!pipeline.applications.length && !pending.length) {
      return [
        { label: t.assistant.chipSetupLabel, send: t.assistant.chipSetupSend },
        { label: t.assistant.chipImproveCvLabel, send: t.assistant.chipImproveCvSend },
      ];
    }
    if (pending.length) {
      const counts = new Map<string, number>();
      ...
      if (top && top[1] > 1)
        chips.push({ label: t.assistant.chipEvaluateAllLabel(top[0], top[1]), send: t.assistant.chipEvaluateAllSend(top[0]) });
      chips.push({ label: t.assistant.chipTriageLabel(pending.length), send: t.assistant.chipTriageSend(pending.length) });
    }
    const strong = pipeline.applications.filter((a) => scoreNum(a.score) >= 4.5).length;
    if (strong) chips.push({ label: t.assistant.chipStrongLabel, send: t.assistant.chipStrongSend });
    chips.push({ label: t.assistant.chipTodayLabel, send: t.assistant.chipTodaySend });
        ...
        <div className="fixed bottom-5 right-5 z-50 flex h-[600px] max-h-[80vh] w-[400px] max-w-[calc(100vw-2.5rem)] flex-col overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl">
          <header className="flex items-center gap-2.5 border-b border-border px-4 py-3">
            ...
            <div className="flex-1">
              <div className="text-sm font-semibold tracking-tight">{t.assistant.panelTitle}</div>
              <div className="text-xs text-faint">{cliId ? t.assistant.via(cliId) : t.assistant.noCli}</div>
            </div>
            <Button variant="ghost" size="icon" onClick={resetChat} className="text-muted" aria-label={t.assistant.newChat} title={t.assistant.newChat}>
              <RotateCcw className="size-4" />
            </Button>
            <Button variant="ghost" size="icon" onClick={() => setOpen(false)} className="text-muted" aria-label={t.assistant.close}>
              <X className="size-4" />
            </Button>
          </header>
          ...
          <div className="border-t border-border p-3">
            <div className="flex items-end gap-2">
              <textarea
                ...
                placeholder={cliId ? t.assistant.askAnything : t.assistant.configureCliFirst}
                rows={1}
                disabled={!cliId}
    ...
    if (!job)
      return (
        <Link href={`/jobs/${part.jobId}`} className="block rounded-xl border border-border bg-surface/40 p-2.5 text-xs text-faint hover:text-foreground">
          {t.assistant.workerFinishedEarlier}
        </Link>
      );
    return (
      <WorkerCard
        ...
        variant="inline"
        trailing={
          <Link href={`/jobs/${job.id}`} className="text-faint transition-colors hover:text-brand" aria-label={t.assistant.openWorker}>
        ...
        <div className="space-y-1.5">
          {children.map((j) => (
            <WorkerCard
              ...
              variant="inline"
              trailing={
                <Link href={`/jobs/${j!.id}`} className="text-faint transition-colors hover:text-brand" aria-label={t.assistant.openWorker}>
                  <ArrowUpRight className="size-3.5" />
                </Link>
              }
            />
    ...
    return (
      <div className="rounded-xl border border-brand/40 bg-brand-soft p-2.5">
        ...
        {part.state === "pending" ? (
          ...
          <div className="mt-1 text-xs text-faint">{part.state === "done" ? t.assistant.started : t.assistant.cancelled}</div>
        )}
      </div>
    );
  }
