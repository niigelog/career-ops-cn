...
export function TodayDashboard({
  ...
  const dateLabel = useMemo(
    () => new Date().toLocaleDateString(intlLocale, { weekday: "long", month: "short", day: "numeric" }),
    [intlLocale],
  );
  ...
  return (
    <div className="mx-auto max-w-none px-6 py-8">
      <section className="dot-bg relative overflow-hidden rounded-2xl border border-border bg-surface/40 px-7 py-10 md:px-10 md:py-12">
        ...
        <div className="relative z-10">
          <p className="font-mono text-xs uppercase tracking-[0.2em] text-muted">
            <span className="text-faint">//</span> {t.home.eyebrowToday} · <span className="tabular-nums">{dateLabel}</span>
          </p>
          <h1 className={`${instrumentSerif.className} mt-3 text-4xl leading-[1.05] text-landing md:text-5xl`}>
            {allClear ? (
              ...
              <>
                {newThisWeek > 0 && (
                  <>
                    {t.home.newMatches(newThisWeek).before}
                    <span className="text-brand tabular-nums">{newThisWeek}</span>
                    {t.home.newMatches(newThisWeek).after}
                  </>
                )}
                {newThisWeek > 0 && overdue > 0 && <span className="text-faint"> · </span>}
                {overdue > 0 && (
                  <>
                    {t.home.followupsDue(overdue).before}
                    <span className="text-brand tabular-nums">{overdue}</span>
                    {t.home.followupsDue(overdue).after}
                  </>
                )}
              </>
            )}
          ...
          <p className="mt-4 max-w-xl text-sm text-muted">
            {allClear ? t.home.subtitleAllClear : t.home.subtitleQueue}
          </p>
          <div className="mt-6 flex flex-wrap gap-2.5">
            <Link href="/explore" className="inline-flex items-center gap-2 rounded-full bg-brand px-5 py-2.5 text-sm font-medium text-brand-foreground transition hover:bg-brand-200 max-sm:min-h-[44px]">
              {t.home.findRoles} <ArrowRight className="size-4" />
            </Link>
      ...
      {followups.length > 0 && (
        <Section icon={Bell} title={t.home.sectionFollowups} hint={t.home.sectionFollowupsHint}>
      ...
      {awaiting.length > 0 && (
        <Section icon={CircleHelp} title={t.home.sectionAwaiting} hint={t.home.sectionAwaitingHint}>
      ...
      {fresh.length > 0 && (
        <Section icon={Sparkles} title={t.home.sectionFresh} hint={t.home.sectionFreshHint}>
      ...
      {allClear && (
        <div className="mt-8 rounded-2xl border border-border bg-surface/30 px-6 py-10 text-center">
          ...
          <p className="mx-auto mt-3 max-w-md text-sm text-muted">
            {t.home.emptyLead} <Link href="/explore" className="text-brand hover:underline">{t.home.emptyFreeScan}</Link> {t.home.emptyMid} <Link href="/pipeline" className="text-brand hover:underline">{t.home.emptyPipeline}</Link>{t.home.emptyEnd}
          </p>
        </div>
      )}
    </div>
