  ...
  return (
    <div className="flex min-w-0 flex-col gap-2.5 rounded-xl border border-border bg-surface/40 p-3.5 transition hover:border-brand/30">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {busy === "Applied" ? <Loader2 className="size-3.5 animate-spin" /> : <Check className="size-3.5" />} {t.cards.markApplied}
        </button>
        <button
          type="button"
          disabled={!!busy}
        ...
        >
          {busy === "Discarded" ? <Loader2 className="size-3.5 animate-spin" /> : <X className="size-3.5" />} {t.cards.skip}
        </button>
        <a href={`/pipeline/${app.n}`} title={t.cards.openReport} aria-label={t.cards.openReport} className="inline-flex shrink-0 items-center justify-center rounded p-1.5 text-faint transition hover:text-brand max-sm:min-h-[44px] max-sm:min-w-[44px]">
