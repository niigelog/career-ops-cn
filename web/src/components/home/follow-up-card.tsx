    ...
    <div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-2 rounded-xl border border-border bg-surface/40 px-3.5 py-3 transition hover:border-brand/30">
      <div className="flex min-w-0 flex-[1_1_55%] items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="flex items-center gap-1 text-[11px] text-faint">
            <Clock className="size-3" /> {followup.appliedDate ? t.cards.appliedOn(followup.appliedDate) : t.cards.followUpDue}
          </p>
        </div>
      </div>
      <div className="ml-auto flex shrink-0 items-center gap-2">
        ...
        >
          ...
          <span className="hidden max-w-48 truncate sm:inline">{state === "error" ? t.cards.retryLong(errorMsg ?? t.cards.failed) : t.cards.markFollowedUp}</span>
          <span className="sm:hidden">{state === "error" ? t.cards.retryShort : t.cards.followedUpShort}</span>
        </button>
        {followup.num != null && (
          <a href={`/pipeline/${followup.num}`} title={t.cards.openReport} className="inline-flex shrink-0 items-center justify-center rounded p-1 text-faint transition hover:text-brand max-sm:min-h-[44px] max-sm:min-w-[44px]">
