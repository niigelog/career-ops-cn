...
export function InboxTriage({ inbox }: { inbox: InboxJob[] }) {
  ...
  const skip = (job: InboxJob) => {
    ...
    setUndo({ label: t.inbox.skipped(job.company), fn: () => setHidden((h) => h.filter((u) => u !== job.url)) });
  };
  ...
  const scoreShortlist = () => {
    ...
    for (const it of shortlist) {
      startJob({ title: t.inbox.scoreTitle(it.company), subtitle: it.role, kind: "evaluate", input: it.url, page: "/pipeline", batchId });
    }
  ...
  return (
    <div className={cn("mx-auto mt-4 max-w-none", shortlist.length > 0 && "pb-28 sm:pb-24")}>
      <FacetChips
        within={within}
        setWithin={setWithin}
        sources={sources}
      ...
      <div className="mt-4 flex items-baseline justify-between gap-3">
        <p className="text-sm font-medium text-foreground">
          {capped ? t.inbox.freshWorthLook : anyFacet ? t.inbox.matchCount(filtered.length) : t.inbox.allRoles}
        </p>
        {hiddenCount > 0 && (
          <button type="button" onClick={() => setHidden([])} className="text-xs text-faint transition-colors hover:text-foreground">
            {t.inbox.hiddenRestore(hiddenCount)}
          </button>
        )}
      </div>
      ...
      {selected.size > 0 && (
        <div className="mt-2 flex items-center gap-3 rounded-lg border border-brand/30 bg-brand-soft px-3 py-2 text-sm">
          <span className="font-medium text-brand tabular-nums">{t.inbox.selectedCount(selected.size)}</span>
      ...
      {capped && ordered.length > BATCH && (
        ...
        >
          {t.inbox.seeAllInInbox(ordered.length)}
        </button>
      )}

      {/* empty-shortlist guidance (only once there's nothing saved) */}
      {shortlist.length === 0 && (
        <p className="mt-4 text-center text-xs text-faint">{t.inbox.shortlistHint}</p>
      )}
