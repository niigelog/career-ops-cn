...
function agoLabel(age: number | null, t: Dict): string | null {
  if (age == null) return null;
  if (age <= 0) return t.inbox.today;
  if (age === 1) return t.inbox.yesterday;
  if (age < 7) return t.inbox.daysAgo(age);
  if (age < 30) return t.inbox.weeksAgo(Math.floor(age / 7));
  return t.inbox.monthsAgo(Math.floor(age / 30));
}
  ...
  return (
    ...
    >
      ...
      <input
        ...
        aria-label={t.inbox.selectRow(job.company, job.role)}
      ...
      {evaluated ? (
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          >
            ...
            <span className="max-sm:hidden">{shortlisted ? t.inbox.savedShort : t.inbox.saveShort}</span>
          </button>
          <button
            type="button"
            onClick={onSkip}
            title={t.inbox.skipHint}
