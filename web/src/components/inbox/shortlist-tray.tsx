...
export function ShortlistTray({
  ...
  const costText = estimate.tokens
    ? t.shortlist.costEstimate(fmtTokens(estimate.tokens), estimate.usd != null ? estimate.usd.toFixed(2) : null)
    : t.shortlist.usesYourTokens;
        ...
        <div className="border-t border-border bg-surface shadow-lg shadow-black/10 sm:rounded-2xl sm:border">
              ...
              {items.map((it) => (
                <li key={it.url} className="flex items-center gap-2.5 py-2">
                  ...
                  <button
                    ...
                    aria-label={t.shortlist.removeItem(it.company)}
          ...
          <div className="flex items-center gap-3 px-3 py-2.5 sm:px-4">
            ...
            >
              ...
              {t.shortlist.title} <span className="tabular-nums text-brand-text">({n})</span>
            </button>
            ...
            <div className="ml-auto flex items-center gap-2">
              {!confirming ? (
                ...
                >
                  ...
                  <span>{t.shortlist.scoreN(n)}</span>
          ...
          <div className="flex items-center gap-2 border-t border-border/60 px-3 py-1.5 text-[11px] text-muted sm:hidden">
            ...
            <span>{t.shortlist.onlyStepThatSpends(costText)}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  if (!hasCli) {
    return (
      <div className="flex items-center gap-2 text-xs">
        <span className="text-muted">{t.shortlist.noAiConfigured}</span>
        <Link href="/config" className="inline-flex items-center gap-1 rounded-full border border-brand/40 bg-brand-soft px-3 py-1.5 font-medium text-brand max-sm:min-h-[44px]">
          <Settings className="size-3.5" /> {t.shortlist.setUp}
        </Link>
