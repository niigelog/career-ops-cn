...
export function ApplyProvider({ children }: { children: React.ReactNode }) {
  ...
  const drive = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (!r.body) {
        setError(t.apply.errAgentStart);
        setStatus("error");
        return;
      }
      ...
      for (;;) {
        ...
        while ((nl = buf.indexOf("\n")) >= 0) {
          ...
          if (ev.t === "step") setDriveSteps((p) => [...p, ev as DriveStep]);
            ...
            finished = true;
            setError(ev.message || t.apply.errNoFillableForm);
            setStatus("error");
          }
        }
      }
      if (generation.current !== gen) return; // left mid-drive
      if (!finished) {
        setError(t.apply.errAgentStopped);
        setStatus("error");
      }
    } catch (e) {
      if (generation.current !== gen) return; // left mid-drive
      ...
      for (;;) {
        ...
        while ((nl = buf.indexOf("\n")) >= 0) {
          ...
          if (ev.t === "log") {
            ...
            if ((ev.count ?? 0) === 0) setError(t.apply.errZeroAnswers);
            else if (ev.truncated) setError(t.apply.errTruncated);
          } else if (ev.t === "error") {
            sawError = true;
            setError(ev.m ? t.apply.errPrefillWith(ev.m) : t.apply.errPrefill);
            setPrefillLog((p) => [...p, `✗ ${ev.m ?? "error"}${ev.raw ? ` — raw tail: ${ev.raw.slice(0, 160)}` : ""}`]);
          }
        }
      }
      if (generation.current !== gen) return; // left mid-prefill
      if (!got && !sawError) setError(t.apply.errNoAnswers);
      setStatus("ready");
    } catch (e) {
      if (generation.current !== gen) return; // left mid-prefill
  ...
  const fill = useCallback(async () => {
    ...
    try {
      ...
      if (d.navigated) setError(t.apply.warnNavigated);
      setStatus("done");
      ...
      if (generation.current !== gen) return; // left mid-fill
      setError(t.apply.errFillFailed);
      setStatus("error");
    }
  }, [answers, fields]);
  ...
  const agentFill = useCallback(async () => {
    ...
    try {
      ...
      if (generation.current !== gen) return; // left mid-fill
      if (!r.body) {
        setError(t.apply.errAgentFillStart);
        setStatus("error");
        return;
      }
      const reader = r.body.getReader();
      ...
      for (;;) {
        ...
        while ((nl = buf.indexOf("\n")) >= 0) {
          ...
          if (ev.t === "step") setDriveSteps((p) => [...p, ev as DriveStep]);
            ...
            setError(ev.message || t.apply.errAgentFill);
            setStatus("error");
          }
        }
      }
