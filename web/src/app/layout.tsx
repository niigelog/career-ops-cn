...
import { dictFor, HTML_LANG } from "@/lib/i18n";
import "./globals.css";

// Resolved per request so the tab title and share description follow the same
// locale as the UI (static `metadata` cannot read the cookie).
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  // Resolve once per render (cookie → config/profile.yml `language.output` →
  // English) and hand it down: <html lang> for assistive tech and the provider
  // for every client component below.
