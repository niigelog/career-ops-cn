...
// Canonical status tokens (templates/states.yml); labels come from the catalog.
...
export default async function Analytics() {
  ...
  const buckets = [
    ...
    { label: t.analytics.bucketUnder3, test: (n: number) => n < 3 },
  ...
  return (
    <div className="mx-auto max-w-none px-6 py-8">
      ...
      <p className="mt-1 text-sm text-muted">{t.analytics.across(total)}</p>

      {/* headline stats */}
      <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <Stat value={total} label={t.analytics.evaluated} />
        <Stat value={avg ? avg.toFixed(2) : "—"} label={t.analytics.avgScore} />
        <Stat
          value={interviews}
          label={t.analytics.interviews}
          hint={interviews === 0 ? t.analytics.interviewsHint : undefined}
        />
        <Stat
          value={offers}
          label={t.analytics.offers}
          hint={offers === 0 ? t.analytics.offersHint : undefined}
        />
      </div>

      <Section title={t.analytics.byStage}>
        {stageCounts.map((s) => (
          <Bar
            key={s.key}
            label={s.label}
      ...
      <Section title={t.analytics.scoreDistribution}>
        {buckets.map((b) => (
          <Bar key={b.label} label={b.label} value={b.n} pct={(b.n / maxBucket) * 100} total={scores.length} />
        ))}
      </Section>

      <Section title={t.analytics.topCompanies} id="companies">
