...
export function relativeDays(daysUntil: number, t: Dict): string {
  if (daysUntil === 0) return t.followups.relToday;
  if (daysUntil === 1) return t.followups.relTomorrow;
  if (daysUntil > 1) return t.followups.relInDays(daysUntil);
  return daysUntil === -1 ? t.followups.relDayAgo : t.followups.relDaysAgo(-daysUntil);
}
